	...
	local function backListener ( event )
 	...
 	local function goToSceneListener ( event )
 		if (event.phase == "began") then
 			audio.dispose(sound)
			sound = nil
			storyboard.gotoScene("lev2", { effect = "zoomInOutFade", time = 400, params = {curScoree = score}})
 		end
 		return true
 	end
...
function placeObjects()
	...
	bg = display.newImage ("Background1.png", width/2, height/2)
	...
	-- pigs
 	...
 	pig2 = createPig(width*.55, height*.3)
	...
	-- bird
 	bird =  display.newImageRect("redbird.png", 32, 32)
	bird.x = slingshot.x

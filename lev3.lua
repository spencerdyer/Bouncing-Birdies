...
-------------------------------------------------------- VARIABLES -------------------------------------------------------
local group, bg, plank, plank2, plank3, box, pig, pig2, ground, bird, tapMsg, score_msg, nextLevel, backButton, backButtonText, slingshot, startTime
	...
	local function backListener ( event )
 	...
 	local function goToSceneListener ( event )
 		if (event.phase == "began") then
 			audio.dispose(sound)
			sound = nil
			storyboard.gotoScene("gameOver", { effect = "zoomInOutFade", time = 400, params = {curScoree = score}})
 		end
 		return true
 	end
...
function placeObjects()
	...
	bg = display.newImage ("Background3.png", width/2, height/2)
	...
	plank2.x = width*.75
	plank2.y = height*.55
	plank3 = display.newImageRect("Plank.png", 180, 20)
	plank3.x = width*.6
	plank3.y = height*.3
	box = display.newImageRect("HollowSquare.png", 40, 40)
	box.x = width*.6
	box.y = height*.2
	...
	-- pigs
 	pig = createPig(width*.90, height*.7)  
 	pig2 = createPig(width*.35, height*.3)
	pig3 = createPig(width*.65, height*.7)
	
	-- bird
 	bird =  display.newImageRect("bluebird.png", 32, 32)
	bird.x = slingshot.x
